...
class ZodDateSchema extends ZodSchema {
  ...
  protected function parseFormat($value, array $attributes) {
    ...
    $this->addError($attributes['message'] ?? "Data must be in the format \"$this->format\"", $attributes['origin'] ?? []);
    $this->stop();
  }
  ...
  protected function parseMin($value, array $attributes) {
    ...
    $this->addError($attributes['message'] ?? "Date must be greater than \"$this->min\"", $attributes['origin'] ?? []);
  }
  ...
  protected function parseMax($value, array $attributes) {
    ...
    $this->addError($attributes['message'] ?? "Date must be less than \"$this->max\"", $attributes['origin'] ?? []);
  }
